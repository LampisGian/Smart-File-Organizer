...
def ensure_storage_paths() -> tuple[Path, Path]:
    base = Path(__file__).resolve().parent /"Old"
    logs_dir = base / "logs"
    ...
    log_path = logs_dir / "moves_logs.log"
    history_path = history_dir / "moved_files_history.json"
    return log_path, history_path


# This class is responsible for the notifications that appear in the top right corner
class Toast:
...
# This class is responsible for the status of the application with matching colours
...
class SmartOrganizerGUI:
    def __init__(self):
        ...
        # ✅ STORAGE PATHS: code/old/history + code/old/logs
        log_path, history_path = ensure_storage_paths()
